...
class Monkey:
    ...
    def do_turn(self, monkeys: dict[int, Monkey], mod_number: int) -> None:
        while self.items:
...
def solve(s: str) -> int:
    ...
    for m in monkeys.values():
        mod_number *= m.test_num
    for _ in range(10000):
        for m in monkeys.values():
            m.do_turn(monkeys, mod_number)
...
@pytest.mark.parametrize(
    ...
    (
        (INPUT_S, EXPECTED),
        (helpers.read_file(os.path.join(THIS_DIR, "input.txt")), 30599555965),
    ),
)
