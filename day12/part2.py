    ...
    while d:
        ...
        for n in helpers.adjacent_4(i.x, i.y):
            ...
            if (
                ...
                d.append(p)
    possible_starts: list[tuple[pos2d, int]] = list()
    for i, e in heights.items():
        cost = cost_to_finish[i]
        if e == 0 and cost is not None:
            possible_starts.append((i, cost))
    possible_starts.sort(key=itemgetter(1))
...
@pytest.mark.parametrize(
    ...
    (
        (INPUT_S, EXPECTED),
        (helpers.read_file(os.path.join(THIS_DIR, "input.txt")), 332),
    ),
)
